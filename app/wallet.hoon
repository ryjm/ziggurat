...
+$  state-0
  $:  %0
      seed=[mnem=tape pass=tape address-index=@ud]
      keys=(map pub=@ux [priv=(unit @ux) nick=tape])  ::  keys created from master seed
...
++  on-poke
  ...
  ++  poke-wallet
    ...
    ?-    -.act
      ...
      =+  %-  ~(gas by *(map @ux [(unit @ux) tape]))
          %+  murn  ~(tap by keys.state)
          |=  [pub=@ux [priv=(unit @ux) nick=tape]]
          ?^  priv  ~
          `[pub [~ nick]]
    ...
    ::
      ...
      %=  state
        transaction-store  ~
        seed  [mnem password.act 0]
        keys  (~(put by -) public-key:core [`private-key:core nick.act])
      ==
    ::
        %derive-new-address
      ::  if hdpath input is empty, use address-index+1 to get next
      =+  (from-seed:bip32 [64 (to-seed:bip39 mnem.seed.state pass.seed.state)])
      =+  %-  derive-path:-
    ...
    ::
      ...
      =+  pub=public-key:core
      =/  keys  (malt ~[[pub [`private-key:core "0xbeef"]]])
      :-  (create-holder-and-id-subs ~(key by keys) (need indexer.state))
      %=  state
        seed    [mnem "" 0]
        keys    keys
...
++  on-peek
  ...
  ::
    ...
    :~  ['pubkey' (tape (scow %ux pub))]
        ['privkey' ?~(priv (tape "") (tape (scow %ux u.priv)))]
        ['nick' (tape nick)]
        :-  'nonces'
        %-  pairs
